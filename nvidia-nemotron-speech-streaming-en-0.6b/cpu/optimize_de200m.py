"""DE-200M variant of optimize.py — wraps the same stages but uses
nemotron_speech_int4_cpu_de200m.json and points at the patched local .nemo."""
...
_LITE_STARTUP = _SCRIPT_DIR / "_lite_conformer_startup.py"

DEFAULT_MODEL = "/datadisks/disk2/nebanfic/hf_models/lite-conformer-de-200m/best_model_fixed.nemo"
DEFAULT_EXPORT_DIR = "build/de_fp32"
DEFAULT_OUTPUT_DIR = "build/de_int4"
CONFIG_NAME = "nemotron_speech_int4_cpu_de200m.json"


def _lite_env():
    env = os.environ.copy()
    env["PYTHONSTARTUP"] = str(_LITE_STARTUP)
    return env
...
_LITE_BOOTSTRAP = (
    "import sys; sys.path.insert(0, '/home/nebanfic'); "
...
def run_export(model_name, export_dir, chunk_size, left_chunks):
    ...
    subprocess.run([
        sys.executable, "-c", _LITE_BOOTSTRAP, str(_EXPORT_SCRIPT),
    ...
    subprocess.run([
        sys.executable, "-c", _LITE_BOOTSTRAP, str(_TOKENIZER_SCRIPT),
        "--model_name", model_name,
        "--output_dir", str(_resolve(export_dir)),
    ], check=True, cwd=str(_SCRIPT_DIR))
